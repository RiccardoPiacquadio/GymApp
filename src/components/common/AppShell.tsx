    ...
    <div className="min-h-screen bg-app-glow">
      <div className="mx-auto flex min-h-screen max-w-md flex-col px-4 pb-28 pt-5">
        <header className="mb-5 flex items-start justify-between gap-4 text-white">
          <div>
            <p className="text-xs uppercase tracking-[0.24em] text-slate-300">GymApp PWA</p>
            <h1 className="mt-1 text-2xl font-semibold text-white">
              {profile ? `Ciao ${profile.displayName}` : "Workout tracker"}
            </h1>
          </div>
          {activeSession ? <span className="pill bg-accent/15 text-white">Sessione attiva</span> : null}
        </header>
        <main className="flex-1">{children}</main>
      </div>
      {location.pathname !== "/profiles" ? (
        <nav className="fixed inset-x-0 bottom-0 border-t border-steel/30 bg-[#151a21]/92 px-4 py-3 backdrop-blur">
              ...
              return (
                <Link
                  ...
                  className={`rounded-2xl px-3 py-3 text-center text-xs font-semibold transition ${
                    isActive ? "bg-accent text-white" : "bg-white/6 text-slate-300"
                  }`}
                >
                  {item.label}
                </Link>
